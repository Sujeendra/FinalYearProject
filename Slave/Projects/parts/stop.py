...
class Stop(object):
    def __init__(self):
        ...
        print("Enterered")
        self.classifier = os.path.join("/home/pi/Project-8-stop-sign/donkeycar/parts/cv/stopsign_classifier.xml")
    ...
    def run(self, throttle,steering, image_array,usermode):
        distance = self.area_to_dist(self.stop_sign_detection(image_array))
        #print("-- Distance: ", distance)
        print(usermode)
        if(usermode=='local'):
            print("it is inside the auto mode now disabling it dynamically")
            usermode='user'
...
#                print("Throttle_coeff: ", self.throttle_coeff)
        #print("== THROTTLE: ", throttle * self.throttle_coeff, " ==",steering*self.steering_coeff)
        
        try:
            return throttle * self.throttle_coeff,steering*self.steering_coeff,usermode
        except:
